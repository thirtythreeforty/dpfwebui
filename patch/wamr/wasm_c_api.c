...
static int engine_instance_count = 0;
...
wasm_engine_delete_internal(wasm_engine_t *engine)
{
    ...
    if ((engine_instance_count > 0) && (--engine_instance_count == 0)) {
...
wasm_engine_new()
{
    return wasm_engine_new_internal(Alloc_With_System_Allocator, NULL);
}
...
{
    return wasm_engine_new_internal(type, opts);
}
...
wasm_store_new(wasm_engine_t *engine)
{
    ...
    /* keep a reference to engine for wasm_store_delete() */
    store->engine = engine;
    ...
    if (!bh_vector_append((Vector *)engine->stores, &store)) {
...
wasm_store_delete(wasm_store_t *store)
{
    ...
    store_count = bh_vector_size((Vector *)store->engine->stores);
    for (i = 0; i != store_count; ++i) {
        wasm_store_t *tmp;

        if (!bh_vector_get((Vector *)store->engine->stores, (uint32)i,
                           &tmp)) {
            break;
        }

        if (tmp == store) {
            bh_vector_remove((Vector *)store->engine->stores, (uint32)i,
                             NULL);
            break;
        }
    }
...
static wasm_trap_t *
wasm_trap_new_internal(const wasm_engine_t *engine,
                       ...
                       const char *default_error_info)
{
    ...
    if (!engine || !engine->stores || !engine->stores->num_elems) {
        return NULL;
    }
    ...
    if (!(instances = engine->stores->data[0]->instances)) {
        goto failed;
    }
...
failed:
    ...
    /* trap -> exception -> trap */
    if (wasm_runtime_get_exception(func->inst_comm_rt)) {
        return wasm_trap_new_internal(func->store->engine, func->inst_comm_rt,
                                      NULL);
    }
    else {
        return wasm_trap_new_internal(func->store->engine, func->inst_comm_rt,
