...
protected:
    ...
    virtual void postMessage(const JSArray& args) = 0;
    virtual void onMessageReceived(const JSArray& args);

    void handleMessage(const JSArray& args);

    typedef std::function<void(const JSArray& args)> MessageHandler;
